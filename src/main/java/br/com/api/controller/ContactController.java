...
public class ContactController {
	...
	@PostMapping
	public ResponseEntity<ContactModel> contactSave(@RequestBody ContactModel contactModel) {
		return ResponseEntity.ok().body(this.servico.contactSave(contactModel));

	}

	@GetMapping
	public ResponseEntity<List<ContactModel>> contactList() {
		return ResponseEntity.ok().body(this.servico.contactList());

	}

	@PutMapping("/{id}")
	public ResponseEntity<ContactModel> contactUpdate(@PathVariable Long id, @RequestBody ContactModel contactModel)
			throws Exception {
